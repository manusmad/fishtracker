...
% Last Modified by GUIDE v2.5 15-Jul-2014 09:32:25
...
function handles = populateTracksList(handles)
    if isfield(handles,'tracks')
        ...
        list = cell(nTracks,1);
        for k = 1:nTracks
            coltag = reshape(dec2hex(round(handles.cTracks(k,:)*255))',1,6);           
...
function saveElecBtn_Callback(hObject, ~, handles)
    if isfield(handles,'elec')
        [elecFileName,elecFilePath] = uiputfile('./*.mat');
        ...
        elseif strcmp(handles.params.viewMode,'Normal')
            ...
            if strcmp(handles.params.viewChannel,'Single')
                ...
                if handles.params.viewTracks && isfield(handles,'tracks')
                    ...
                    [plotFlag,handles.hTracks,handles.cTracks] = plotTracks(handles.hSingle,handles.tracks,selTracks);
                end
            ...
            elseif strcmp(handles.params.viewChannel,'All')
                for k = 1:handles.meta.nCh      % For each channel
                    ...
                    if handles.params.viewTracks && isfield(handles,'tracks')
                        ...
                        [plotFlag,handles.hTracks,handles.cTracks] = plotTracks(handles.hSub(k),handles.tracks,selTracks);
                    end
...
function trackBtn_Callback(hObject, ~, handles)
    if isfield(handles,'spec')
        ...
        handles.nTracks = length(unique([handles.tracks.id]));
...
function loadElecBtn_Callback(hObject, ~, handles)
    [elecFileName,elecFilePath] = uigetfile('./*.mat');
    try
...
function loadSmrBtn_Callback(hObject, ~, handles)
    [smrFileName,smrFilePath,~] = uigetfile('./*.smr');
...
function loadSpecBtn_Callback(hObject, ~, handles)
    [specFileName,specFilePath] = uigetfile('./*.mat');
    try
...
function saveSpecBtn_Callback(hObject, ~, handles)
    if isfield(handles,'spec')
        [specFileName,specFilePath] = uiputfile('./*.mat');
...
function loadTracksBtn_Callback(hObject, ~, handles)
    if isfield(handles,'tracksFilePath')
        [tracksFileName,tracksFilePath] = uigetfile([handles.tracksFilePath filesep '*.mat']);        
    else
        [tracksFileName,tracksFilePath] = uigetfile('./*.mat');
    end
...
%     try
        ...
        handles.nTracks = length(unique([tracks.id]));
        set(handles.nTracksTxt,'String',num2str(handles.nTracks));
...
function saveTracksBtn_Callback(hObject, ~, handles)
    if isfield(handles,'tracks')
        [tracksFileName,tracksFilePath] = uiputfile('./*.mat');

        tracks = handles.tracks;
        progressbar('Saving tracks data');
        tic;
        save(fullfile(tracksFilePath,tracksFileName),'tracks');
        runTime = toc;
        progressbar(1);

        handles = writeLog(handles,'Saved data file %s (%.2f s)',tracksFileName,runTime);
        set(handles.elecFileTxt,'String',tracksFileName);
...
% --- Executes on button press in addPointBtn.
function addPointBtn_Callback(hObject, eventdata, handles)
% hObject    handle to addPointBtn (see GCBO)
