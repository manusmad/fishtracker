...
trialFolders = {'data_lab_2014_04_06_singleFreeTrials','data_lab_2014_04_22_threeFreeTrials'};
...
for k = 1:length(trialFolders)
    ...
    for j = 1:length(particleFileNames)
        ...
        for n = 1:vidTracked.nFish
            V(n).x = (vidTracked.fishCen(:,1,n)-vidTracked.gridcen(5,1))/6;
            V(n).y = (vidTracked.fishCen(:,2,n)-vidTracked.gridcen(5,2))/6;
            V(n).theta = vidTracked.fishTheta(:,n);
            
            E(n).x = particle.fish(n).x(timeIdx);
            E(n).y = particle.fish(n).y(timeIdx);
            E(n).theta = particle.fish(n).theta(timeIdx); 
        end
...
edges_xy = 0:d_xy:max(e_xy_data_all);
...
subplot(2,1,2), hold on;
    ...
    set(gca,'FontSize',15)
hold off;
export_fig('cumulative_error_free.pdf','-pdf','-nocrop','-painters')
...
export_fig('shuffled_rms_error_free.pdf','-pdf','-nocrop','-painters')
